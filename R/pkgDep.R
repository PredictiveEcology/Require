...
#   out <- Map(packages = deps, f = getPkgDeps, MoreArgs = list(repos, verbose, type, which, purge))
...
# }}
#
#     saveNamesRecursiveFalse <- saveNames
#       # saveNamesForCache(packages, which, recursive = FALSE, ap = ap, repos = repos,
#       #                   verbose = verbose - 1)
...
#     set(saveNamesDT, NULL, "origNames", names(packages))
...
#     neededFull1 <-
#       lapply(saveNames, get0, envir = pkgDepDepsEnv())
...
#     names(packageFullNamesToGet) <- saveNamesDT$origNames[needGet]
#     if (any(needGet)) {
#       fn <- pkgDepDBFilename()
#       if (length(fn)) {
...
#
#       pkgDepDTList <- lapply(saveNamesRecursiveFalse, get0, envir = pkgDepDepsEnv())
#       # pkgDepDTList <- mget(saveNamesRecursiveFalse, envir = pkgDepDepsEnv())
#       names(pkgDepDTList) <- names(saveNamesRecursiveFalse)
#       nulls <- sapply(pkgDepDTList, is.null)
#       if (any(nulls)) {
#         pkgDepDTListTmp <-
#           pkgDepInnerMemoise(
#             packages = pkgDT[nulls], # packageFullNamesToGet[nulls],
#             libPath = libPath,
#             packageNames = names(packageFullNamesToGet[nulls]),
#             which = which[[1]], keepVersionNumber = keepVersionNumber,
#             purge = FALSE, repos = repos,
#             verbose = verbose, includeBase = includeBase,
#             type = type, ap = ap
#           )
#         names(pkgDepDTList)[nulls] <- pkgDepDTListTmp
#       }
...
#       theNulls <-
#         unlist(lapply(pkgDepDTList, function(x) {
#           is.null(x) || NROW(x) == 0
#         }))
#       pkgDepDTListNotRecursive <- pkgDepDTList[!theNulls]
...
#           pkgDepDT <- rbindlist(pkgDepDTListNotRecursive, fill = TRUE, use.names = TRUE)
#           ap <- getAvailablePackagesCheckAdditRepos(pkgDepDTListNotRecursive, pkgDepDT, repos, verbose, type)
#           pkgDepDTListRecursive <-
#             Map(
#               needed = pkgDepDTListNotRecursive, counter = seq_len(NROW(pkgDepDTListNotRecursive)),
#               function(needed, counter) {
#                 i <- 1
#                 pkgDepDTListNew <- pkgsNew <- list()
#                 pkgsNew[[i]] <- needed$packageFullName
#                 pkgDepDT3 <- needed
#                 news <- needed
#                 if (!all(news$Package %in% .basePkgs)) {
#                   while (TRUE) { # use a break below
#                     # while (length(unlist(pkgsNew[[i]])) > 0) {
#                     i <- i + 1
#                     pkgsToLookupFull <-
#                       news$packageFullName
...
#                           packages = need, libPath = libPath, which = which, purge = FALSE,
#                           keepVersionNumber = keepVersionNumber, ap = ap,
#                           repos = repos, verbose = verbose, includeBase = includeBase
#                         )
#                         if (is(a, "try-error")) browserDeveloper("Error 23")
#                         a
#                       })))
#
#                     ap <- getAvailablePackagesCheckAdditRepos(pkgDepDTListNew, repos = repos, type = type, ap = ap)
#                     news <- unique(rbindlist(pkgDepDTListNew, fill = TRUE, use.names = TRUE))
#                     newPkgs <- !news$packageFullName %in% pkgDepDT3$packageFullName
#                     if (any(newPkgs)) {
#                       news <- news[newPkgs,]
#                       set(news, NULL, "recursion", i - 1 )
#                       pkgDepDT3 <- rbindlist(list(pkgDepDT3, news), use.names = TRUE, fill = TRUE)
#                     } else {
#                       break
#                     }
#                   }
...
#                   if (messageIfGTN) {
#                     if (counter == 1) ttimes <<- data.table(counter, Sys.time(), paste(news$packageFullName, collapse = ", "))
#                     ttimes <<- rbindlist(list(ttimes, data.table(counter, Sys.time(), paste(news$packageFullName, collapse = ", "))))
...
#           packageFullName <- gsub(.grepTooManySpaces, "", packageFullName)
#           packageFullName <- gsub(paste0("([[:alnum:]]{1})(\\()"), "\\1 \\2", packageFullName)
#           packageFullName
#         }]})
#
#       # Add deps to pkgDepDepsEnv()
#       assignPkgDepDTtoSaveNames(sn = saveNamesDT$saveNames[newOnes], pkgDepDT = pkgDepDTList2)
#       # Map(sn = saveNamesDT$saveNames[newOnes], n = saveNamesDT$names[newOnes], function(sn, n) {
#       #   assign(sn, pkgDepDTList2[[n]], envir = .pkgEnv[[.pkgDepEnvName]][["deps"]])
#       # })
...
#         DT[, packageFullName := trimVersionNumber(packageFullName)])
#     }
#     if (!isTRUE(includeBase)) {
#       # neededFull1 <- lapply(neededFull1, setdiff, .basePkgs)
#       neededFull1 <- lapply(neededFull1, function(DT)
#         DT[!packageFullName %in% .basePkgs])
#     }
#     if (messageIfGTN) {
...
pkgDepInner <- function(packages,
  ...
  pkgDepDTList <- Map(pkg = packagesToCheck,
    pkgName = names(packagesToCheck),
    pkgNoVersion = pkgsNoVersionToCheck,
    MoreArgs = list(verbose = verbose, which = which, includeBase = includeBase,
                    keepVersionNumber = keepVersionNumber, repos = repos,
                    type = type, ap = ap),
    f = pkgDepInnerByPkg
  ) # a list of pkgDepDT
  if (!isTRUE(includeBase)) {
    if (NROW(packagesBase)) {
      pkgDepDTList <- append(pkgDepDTList, packagesBase)
      pkgDepDTList <- pkgDepDTList[match(names(packages), names(pkgDepDTList))]
    }
  }
  isDT <- vapply(pkgDepDTList, is.data.table, FUN.VALUE = logical(1))
  if (any(!isDT))
    pkgDepDTList[!isDT] <- lapply(pkgDepDTList[!isDT], function(x) data.table::data.table(packageFullName = character()))
...
pkgDepInnerMemoise <- function(...) {
  ...
  if (getOption("Require.useMemoise", TRUE)) {
    ...
    packageNames <- eval(ss$packageNames, envir = parent.frame())

    if (NROW(packages) > 1) {
      dots2 <- dots
      dots2[[1]] <- NULL # get rid of packages ... already have locally
      dots2$packageNames <- NULL
      ...
      # }
      if (!is.data.table(packages)) {
        packages <- toPkgDT(packages)
      }
      ret <- packages[, pkgDepInnerMemoise(.SD), by = "Package"]

      # ret <-
      #   Map(p = packages, pn = packageNames, function(p, pn) {
      #     do.call(pkgDepInnerMemoise, append(list(packages = p, packageNames = pn), dots2))
      #   })
      ...
      if (is.null(ret)) {
        if (is.null(ret)) {
          ret <- pkgDepInner(...) # uses pkgDepCRANInner
        } else {
          ret <- list(ret)
          names(ret) <- packages
        }
...
pkgDepInnerByPkg <- function(pkgDT, verbose, which, includeBase, repos, type, ap) {
  # names(pkg) <- pkgName
  #
  ...
  if ("GitHub" %in% pkgDT$repoLocation) {
    ...
    if (!isTRUE(getOption("Require.offlineMode"))) {
      ...
      if (is.null(needed)) {
        ...
        if (!dir.exists(packageTD)) { # only happens in a new session
          ...
          if (!dir.exists(packageTD)) {
            ...
            if (NROW(dt))
              if (noSpecNeeded %in% FALSE)
                dt <- dt[grep(verNum, PackageUrl)]
            PackageUrl <-if (NROW(dt)) {
            ...
            if (length(PackageUrl) > 0) {
              if (endsWith(PackageUrl, "tar.gz")) {
                ...
                localFileOption <- dir(Require::RequirePkgCacheDir(),
                                       pattern = paste0("^", pkgName, "_", verNum, "\\."), full.names = TRUE)
                ...
                if (any(localOption)) {
                  ...
                  if (any(theZip)) {
                    unzip(localFileOption[theZip][1], exdir = td, files = file.path(pkgName, "DESCRIPTION"))
                  } else {
                    untar(tarfile = localFileOption[theTar][1], exdir = td, files = file.path(pkgName, "DESCRIPTION"))
                  }
                } else {
                  messageVerbose(
      ...
      purge <<- FALSE
      pkgDepDT <- toPkgDepDT(needed, needed, needed, verbose)
    } else {
      pkgDepDT <- toPkgDepDT(NULL)
    }
  }
  sn <-
  ...
  pkgDepDTList <- list(pkgDepDT)
  names(pkgDepDTList) <- names(sn)
  assignPkgDepDTtoSaveNames(sn, pkgDepDTList)
  pkgDepDT
}
