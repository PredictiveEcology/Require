...
installFrom <- function(pkgDT, purge = FALSE, repos = getOption("repos"),
  ...
  if (!is.null(rpackageFolder(getOptionRPackageCache()))) {
    ...
    localFiles <- dir(rpackageFolder(getOptionRPackageCache()), pattern = ".zip|.tar.gz", full.names = TRUE)
    # sanity check -- there are bad files, quite often, also directories
    # fs <- file.size(localFiles)
...
doInstalls <- function(pkgDT, install_githubArgs, install.packagesArgs,
  ...
  if (any(doPackages) && (isTRUE(install) || install == "force")) {
    ...
    if (any(!toInstall$installFrom %in% c("Fail", "Duplicate"))) {
      ...
      topoSorted <- pkgDepTopoSortMemoise(pkgs = toInstall$packageFullName, returnFull = TRUE, verbose = verbose, purge = FALSE)
      ...
      if (any(namespacesLoaded) && getOption("Require.unloadNamespaces", TRUE)) {
        ...
        topoSortedAllLoaded <- try(names(pkgDepTopoSortMemoise(pkgs = allLoaded, verbose = verbose, purge = FALSE)))
