utils::globalVariables(c(
  ...
  "github",
  "groupCRANtogether",
  "groupCRANtogetherChange",
  "groupCRANtogetherDif",
  "hasVersionSpec",
  "i.neededFiles",
  "inequality",
  "installFrom",
...
parseGitHub <- function(pkgDT, verbose = getOption("Require.verbose")) {
  ...
  if (is.null(pkgDT$githubPkgName)) {
    ...
    if (any(pkgDT$repoLocation == .txtGitHub)) {
      ...
      if (any(pkgDT$hasSubFolder, na.rm = TRUE)) {
        # fix both Account and RepoWBranch
...
DESCRIPTIONFileVersionV <- function(file, purge = getOption("Require.purge", FALSE)) {
  if (is.null(envPkgDepDESCFile())) {
    purge <- dealWithCache(purge, checkAge = FALSE)
  }
  out <- lapply(file, function(f) {
    out <- if (!is.null(envPkgDepDESCFile())) {
      if (purge && length(f) == 1) {
        suppressWarnings(rm(f, envir = envPkgDepDESCFile()))
      }
      if (length(f) == 1) {
...
dlGitHubFile <- function(
  ...
  ret <- if (NROW(pkg) > 0) {
    ...
    if (is.null(pkgDT[["hasSubFolder"]])) {
      set(pkgDT, NULL, "hasSubFolder", FALSE)
    }
    pkgDT[
      repoLocation == .txtGitHub,
      url := {
        gitHubFileUrl(
    ...
    if (isTRUE(any(feDF))) {
      ...
      if (isTRUE(any(hasNonHead))) {
        versionLocalOK <- compareVersion2(
          versionLocal[hasNonHead],
    ...
    if (!isTRUE(getOption("Require.offlineMode"))) {
      ...
      if (any(!alreadyExists)) {
        ...
        pkgDT[
          which(repoLocation == .txtGitHub & alreadyExists %in% FALSE),
          filepath := .downloadFileMasterMainAuthWithHandlers(
            Account,
            Package,
            Branch,
            packageFullName,
...
dlArchiveVersionsAvailable <- function(
  package,
  ...
  info <- lapply(info, function(dd) {
    lapply(dd, function(d) as.data.table(d, keep.rownames = "PackageUrl"))
  })
...
installedVers <- function(pkgDT, libPaths) {
  ...
  if (NROW(pkgDT)) {
    ...
    ip <- as.data.table(.installed.pkgs(
      ...
      other = "LibPath",
      which = NULL,
      packages = pkgDT$Package
    )) # , other = c("Package", "Version"))) # these 2 are defaults
    ...
    if (NROW(ip)) {
      ...
      if (NROW(pkgs)) {
        ...
        installedPkgsCurrent[,
          VersionFromPV := tryCatch(
            {
...
available.packagesCached <- function(
  repos,
  purge,
  verbose = getOption("Require.verbose"),
  ...
  reposShort <- paste(
    substr(
      unlist(lapply(strsplit(reposNoHttp, "//"), function(x) {
        tryCatch(x[[1]], silent = TRUE, error = function(e) {
          tryCatch(getOption("repos"), silent = TRUE, error = function(f) "unknown")
        })
      })),
      1,
      20
...
isBinaryCRANRepo <- function(
  curCRANRepo = getOption("repos")[["CRAN"]],
...
detachAll <- function(pkgs, dontTry = NULL, doSort = TRUE, verbose = getOption("Require.verbose")) {
  messageVerbose(
    "Detaching is fraught with many potential problems; you may have to ",
  ...
  if (length(dontTryExtra)) {
    messageVerbose(
      "some packages don't seem to unload their dlls correctly. ",
      "These will not be unloaded: ",
      paste(dontTryExtra, collapse = comma),
      verbose = verbose,
      verboseLevel = 2
    )
...
warningCantInstall <- function(pkgs, libPaths = .libPaths()) {
  warning(
    "Can't install ",
    pkgs,
    "; you will likely need to restart R and run:\n",
    "-----\n",
    "install.packages(c('",
...
preparePkgNameToReport <- function(Package, packageFullName) {
  ...
  pkgNameInPkgFullName <- unlist(Map(pkg = Package, pfn = packageFullName, function(pkg, pfn) {
    grepl(pkg, pfn)
  }))
      ...
      pkgGitHub[Package %in% pk, {
        ...
        if (isFALSE(leaveAlone)) {
          ...
          if (any(dups)) {
            ...
            #   version
            dupsRev <- duplicated(vapply(
              strsplit(rev(txt), split = "\\:"),
              function(x) x[[1]],
              FUN.VALUE = character(1)
            ))
...
downloadRepo <- function(
  ...
  out <- suppressWarnings(try(
    .downloadFileMasterMainAuth(url, destfile = zipFileName, need = "master"),
    silent = TRUE
  ))
  if (is(out, "try-error")) {
    return(out)
  ...
  if (is(badDirname, "try-error")) {
    stop("Error 654; something went wrong with downloading & building the package")
  }
  ...
  newName <- unlist(Map(
    ...
    function(bad, subFolder, pkgName) {
      ...
      if (!identical(actualFolderName, pkgName)) {
        # means the folder is not the pkgName e.g., mumin != MuMIn
        origOut <- normPath(out)
        outNP <- origOut
        newFolder <- dirname(bad)
        newFolder <- file.path(newFolder, pkgName)
        if (!isFALSE(subFolder)) {
          # get rid of subfolder for all files
  ...
  messageVerbose(
    paste0(
      gitRepo,
      " downloaded and placed in ",
      normalizePath(repoFull, winslash = "/"),
      collapse = "\n"
    ),
    verbose = verbose,
    verboseLevel = 2
...
getSHAfromGitHub <- function(acct, repo, br, verbose = getOption("Require.verbose")) {
  ...
  for (ii in 1:4) {
    ...
    if (downloadNow) {
      mess <- capture.output(
        type = "message",
        out <- .downloadFileMasterMainAuth(gitRefsURL, destfile = tf, need = "master")
      )
    }
    fetf <- file.exists(tf)
    ...
    for (branch in br) {
      # will be length 1 in most cases except master/main
      whHasBr <- which(vapply(
        gitRefsSplit2,
        function(xx) {
...
getSHAfromGitHubMemoise <- function(...) {
  ...
  if (getOption("Require.useMemoise", TRUE)) {
    ...
    if (
      !exists(uniqueID, envir = pe[[.txtGetSHAfromGitHub]], inherits = FALSE) &&
    ...
    if (!exists(uniqueID, envir = pe[[.txtGetSHAfromGitHub]], inherits = FALSE)) {
      ...
      whIdent <- unlist(lapply(pe[[.txtGetSHAfromGitHub]][[uniqueID]], function(x) {
    ...
    if (is.null(ret)) {
      # Case where it doesn't exist in pe
      ...
      if (length(fn)) {
        # this can be character() if cacheGetOptionCachePkgDir() is NULL
...
loadGitHubSHAsFromDisk <- function(verbose = getOption("Require.verbose")) {
  ...
  if (!exists(.txtGetSHAfromGitHub, envir = pe, inherits = FALSE)) {
    ...
    if (isTRUE(file.exists(fn))) {
      ...
      if (!removeFile) {
        # remove if 24 hours old
        # if (!exists(.txtGetSHAfromGitHub, envir = pe, inherits = FALSE))
        pe[[.txtGetSHAfromGitHub]] <- new.env()
        list2env(out, envir = pe[[.txtGetSHAfromGitHub]])
      } else {
        messageVerbose(
          "Purging disk-backed pkgDep cache for GitHub SHA values; it has ",
          "been more than ",
...
sourcePkgs <- function(additional = NULL, spatialPkgs = NULL, otherPkgs = NULL) {
...
#' @export
.downloadFileMasterMainAuth <- function(
  url,
  destfile,
  need = "HEAD",
  verbose = getOption("Require.verbose"),
  ...
  if (!all(hasMasterMain)) {
    if (length(url) > 1) {
      stop("This function is not vectorized")
    }
  ...
  ret <- withCallingHandlers(
    {
      for (i in 1:2) {
        if (!is.null(urls[["FALSE"]])) {
          outNotMasterMain <- Map(
            URL = urls[["FALSE"]],
            MoreArgs = list(df = destfile),
            function(URL, df) {
              for (tryNum in 1:2) {
                if (!isTRUE(getOption("Require.offlineMode"))) {
                  if (is.null(token)) {
                    tryCatch(
                      {
                        if (!dir.exists(dirname(df))) {
                          dir.create(dirname(df), recursive = TRUE)
                        }
                  ...
                  if (is.null(token)) {
                    URL <- stripGHP(ghp, URL)
                  } # this seems to be one of the causes of failures -- the GHP sometimes fails
                }
              }
              return(invisible())
            }
        ...
        if (!is.null(urls[["TRUE"]])) {
          # should be sequential because they are master OR main
          for (wh in seq(urls[["TRUE"]])) {
            if (!isTRUE(getOption("Require.offlineMode"))) {
              if (is.null(token)) {
                outMasterMain <- try(
                  download.file(urls[["TRUE"]][wh], destfile = destfile, quiet = TRUE),
                  silent = TRUE
                )
              } else {
...
extractPkgNameFromWarning <- function(x) {
  if (
    any(grepl(.txtMsgIsInUse, x)) || # "in use"
      any(grepl(.txtInstallationPkgFailed, x))
  ) {
...
availablePackagesCachedPath <- function(repos, type) {
  file.path(
    dirname(cacheGetOptionCachePkgDir()),
    paste0(gsub("https|[:/]", "", repos), collapse = "/"),
    type,
    "availablePackages.rds"
  )
}
...
installPackagesWithQuiet <- function(ipa, verbose) {
  ...
  if (
    isWindows() && identical(ipa$type, "source") && getOption("Require.installPackagesSys") == 0
  ) {
    op <- options(Ncpus = 1)
  ...
  if (getOption("Require.installPackagesSys") && requireNamespace("sys", quietly = TRUE)) {
    for (i in 1:1) {
      ...
      messageVerbose(
        "Failed installation for: ",
        paste(pkgName, collapse = ", "),
        "\nTrying again ... ",
        verbose = verbose
      )
      ipa$pkgs <- pkgName
...
packageFullName <- function(pkgDT) {
  ...
  if (all(colnames(pkgDT) %in% c("GithubRepo", "GithubUsername"))) {
    ifelse(
      !is.na(pkgDT$GithubRepo) & nzchar(pkgDT$GithubRepo),
      paste0(pkgDT$GithubUsername, "/", pkgDT$Package, "@", pkgDT$GithubSHA1),
      paste0(
        pkgDT$Package,
        ifelse(
          is.na(pkgDT$Version) | is.na(pkgDT$inequality),
          "",
    ...
    ifelse(
      !is.na(pkgDT$Repo) & nzchar(pkgDT$Repo),
...
setOfflineModeTRUE <- function(verbose = getOption("Require.verbose")) {
  if (!isTRUE(getOption("Require.offlineMode"))) {
    if (!internetExists()) {
      options("Require.offlineMode" = TRUE, "Require.offlineModeSetAutomatically" = TRUE)
      messageVerbose(
        "Internet appears to be unavailable; setting options('Require.offlineMode' = TRUE)",
        verbose = verbose
      )
    }
  }
...
checkAutomaticOfflineMode <- function() {
  if (getOption("Require.offlineModeSetAutomatically", FALSE)) {
    options("Require.offlineModeSetAutomatically" = NULL, Require.offlineMode = FALSE)
  }
}
...
GETWauthThenNonAuth <- function(url, token, verbose = getOption("Require.verbose")) {
  ...
  if (grepl("Bad credentials", a) || grepl("404", httr::http_status(a)$message)) {
    if (grepl("Bad credentials", a)) {
      messageVerbose(
        red("Git credentials do not work for this url: ", url, "\nAre they expired?"),
        verbose = verbose
      )
    }
    a <- httr::GET(url, httr::add_headers())
...
available.packagesWithCallingHandlers <- function(
  repo,
  type,
  verbose = getOption("Require.verbose")
) {
  ...
  for (attmpt in 1:2) {
    ...
    if (is(out, "try-error") || any(SSLwarns)) {
      ...
      if (any(grepl("cannot open URL", warns)) && attmpt == 1) {
        # seems to be transient esp with predictiveecology.r-universe.dev
        next
      }
      if (urlExists("https://www.google.com")) {
        # this means that the repository does not have the packages.RDS file, meaning it doesn't have e.g., binary packages for R 4.2
        break
      }
      setOfflineModeTRUE(verbose = verbose)
...
getGitCredsToken <- function() {
  token <- tryCatch(gitcreds::gitcreds_get(use_cache = FALSE), error = function(e) NULL)
  if (!is.null(token)) {
...
readLinesWithHandlers <- function(fff) {
  if (length(fff) == 1) {
    withCallingHandlers(lines <- try(readLines(fff), silent = TRUE), warning = function(w) {
      if (
        grepl(
          paste(
...
.downloadFileMasterMainAuthWithHandlers <- function(
  Account,
  Package,
  Branch,
  packageFullName,
  ...
  ret <- NA
  dl <- try(.downloadFileMasterMainAuth(
    unique(url)[1],
    unique(destFile)[1],
    need = "master",
    verbose = verbose
...
stopOnGitRefsFails <- function(gitRefs, fetf, tf, acct, repo, br) {
  isNotFound <- ((NROW(gitRefs) <= 5) &&
    any(grepl("Not Found", gitRefs)) ||
