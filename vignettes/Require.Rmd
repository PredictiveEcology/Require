...
By default, as of version 2.0.0, `Require` uses `pak` to do calculate package dependency trees and installations. However, `Require` applies a different philosophy to package management. The two tools answer the same question — "what should be installed?" — in different ways.
...
`Require` resolves the conflict for you, using the above documented priority:
