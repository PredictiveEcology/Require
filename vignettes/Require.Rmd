...
`Require` uses statement about *version* as the top level priority. Any request to install a package without a version statement will only install a package if it is not installed. Otherwise, it will install nothing. Examples:
...
By default, as of version 2.0.0, `Require` uses `pak` to do the majority of the work, but applies a different philosophy to package management. The two tools answer the same question — "what should be installed?" — in different ways.
...
* **`Require` is stability-first.** If the installed version satisfies your request, `Require` does nothing. It will only install or upgrade when the version constraint you wrote actually requires it.

This is what makes `Require` "set-and-forget". You can put a `Require::Install(...)` line near the top of a script, run that script every day for a year, and your packages will not silently change underneath you. They only change when you change the code.
...
`Require` accepts the full set of R-style constraints right in the call, mixed freely:
...
`Require` resolves the conflict for you, using a documented priority:

1. Honour any version constraint that's been written down,
2. Prefer the CRAN-like version if it satisfies the version constraint,
3. Use the order in which packages were listed.
...
It is tempting to assume `pak`'s own cache would handle a snapshot install end-to-end — hand `pak` a list of `cran::pkg@version` refs and let `pkgcache` deduplicate. We measured this directly. Result, on a 379-package snapshot, cache-warm:
...
`Require` can install **and** load packages with no internet, as long as they (or compatible builds of them) were downloaded once before. Set:
...
`Require` looks for each package in the local `pak` cache and installs from there. Linux PPM binaries, macOS `.tgz`, Windows `.zip`, and source tarballs are all handled. With a warm cache, installs are near-instant — no rebuild, no download.

`pak` provides the cache (via `pak::cache_list()`); the install step uses `install.packages(<file>, repos = NULL)` directly. Calling `pak::pak()` for a local file rebuilds vignettes (which needs the network) and, on Windows, treats PPM binary builds as out of date and re-downloads — so its installer is unsuitable offline. `install.packages` simply runs `R CMD INSTALL` on the tarball: no resolution, no rebuild, no network.
...
3. **Multiple, apparently incompatible requests for the same package don't error.** If a version requirement decides it, that wins. Otherwise CRAN wins. Otherwise the first listed wins.
...
Some of the features make it fast the first time being used on a system, some make it fast the second & subsequent time on a system (which can be first time in a new project). These features are caching and parallel downloads (the latter via `pak`).

## Caching

`Require` inherits `pak` caching, if `Require.usePak = TRUE)`, and adds a few others. 

### Inherited from `pak`

`Require` (via `pak` if `Require.usePak = TRUE)`) keeps a local cache of: package files (source or binary, including binaries it has built locally from source); the package dependency tree (per session). Together, these speed up installation on a computer that can access the local cache, e.g., for each new project. 
