...
| -------------------------------- | :------------------------------: | :-----------------------------------: |
| Archived package*                 | Automatic         | Must prefix with `url::` and exact url path  | 
...
| Package install by version    | Yes | Yes, but doesn't deal well with multiple packages with specific versions |   
...
The following doesn't work with `pak` because BioSIM, a dependency on GitHub is not found. This may be because the package name is not the repository name, but it is not clear from the error message why: 
...
`Require` and `pak` both create local caches that speeds up the installation of packages on a computer that can access the local cache, e.g., for each new project. Currently, the caches of the two packages take slightly different approaches, but the results are similar, with a key exception (currently, in `pak` version `7.2.1`) of packages built from source. `Require` keeps the binary once the `source` package is built, and it can therefore install the binary each subsequent installation. This results in dramatically faster installations using `Require` when source packages are involved.
