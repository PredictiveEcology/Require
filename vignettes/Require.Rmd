...
* **Network probes are suppressed.** `pak` normally fetches `bioconductor.org/config.yaml` and refreshes its repo metadata at startup, even when you only want a local install. `Require` sets the right environment variables so those calls are skipped for the duration of the install.
