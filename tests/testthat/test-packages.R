...
#     suppressWarnings(try(detach("package:Holidays", unload = TRUE), silent = TRUE))
#     suppressWarnings(try(detach("package:TimeWarp", unload = TRUE), silent = TRUE))
#     suppressWarnings(try(detach("package:latdiag", unload = TRUE), silent = TRUE))
#     suppressWarnings(try(detach("package:meow", unload = TRUE), silent = TRUE))
#   }, add = TRUE)
#
#   packageDir <- tmpdir
...
#   suppressWarnings(try(detach("package:TimeWarp", unload = TRUE), silent = TRUE))
#   # Require("TimeWarp", libPath = packageDir, standAlone = TRUE)
#   # ._Require_1 <<- 1
#   #._installPackages_1 <<- ._installPackages_2 <<- ._installPackages_3 <<- ._Require_1 <<- 1
#   #._installPackages_5 <<- 1
#   Require("Holidays", libPath = c(packageDir, packageDir1), standAlone = FALSE)
#   expect_true(is.na(installedVersions("TimeWarp", packageDir))) # should
#   detach("package:Holidays", unload = TRUE)
#
#   # with standAlone TRUE, both TimeWarp
#   Require("Holidays", libPath = packageDir1, standAlone = TRUE)
#   expect_true(!is.na(installedVersions("TimeWarp", packageDir1)))
#
...
#   depsForHoliday <- pkgDep("Holidays", recursive = FALSE, libPath = packageDir)
#   pkgDeps <- sort(c("Holidays", unique(unlist(depsForHoliday))))
#   skip_on_os("mac")
#   # Alex debug on mac
#   expect_true(all(pkgDeps %in% installed$instPkgs))
#
#   if (FALSE) { # to do -- fix this section of tests
