  ...
  if (isDevAndInteractive && !isMacOS()) {
    ...
    if (getRversion() <= "4.2.3") {
      ...
      download.file(
        file.path(
          rawGithubDotCom,
          "PredictiveEcology/LandR-Manual/30a51761e0f0ce27698185985dc0fa763640d4ae/packages/pkgSnapshot.txt"
        ),
      ...
      pkgs[
        Package %in% "SpaDES.core",
        `:=`(
          Version = "1.1.1",
          GithubRepo = "SpaDES.core",
          GithubUsername = "PredictiveEcology",
      ...
      # remove.packages(pks)
      # unlink(dir(cacheGetOptionCachePkgDir(), pattern = paste(pks, collapse = "|"), full.names = TRUE))
