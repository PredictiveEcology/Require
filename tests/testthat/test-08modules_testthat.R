test_that("test 8", {
  ...
  if (isDevAndInteractive) {
    ...
    if (isWindows()) {
      (Install(
        "Require",
        repos = "https://predictiveecology.r-univierse.dev",
        install = "force"
      )) |>
        capture_warnings() -> warns1
      (a <- Install(
        c("PredictiveEcology/SpaDES.project@development"),
        upgrade = FALSE,
        returnDetails = TRUE
      )) |>
    ...
    # Install modules
    getFromNamespace("getModule", "SpaDES.project")(
    ...
    opts <- options(
      repos = unique(c("https://predictiveecology.r-universe.dev", getOption("repos")))
    )
    on.exit(options(opts), add = TRUE)

    warns <- capture_warnings(deps <- pkgDep(pkgsShort, recursive = TRUE))

    if (Sys.info()["user"] == "emcintir") {
      # source install fails; make sure it is gone
    ...
    otherPkgs <- c(
      "archive",
      "details",
      "DBI", # "s-u/fastshp", # can't compile fastshp in Windows R 4.5
      "logging",
      "RPostgres",
      "slackr"
    )
    ...
    allInstalled <- setdiff(
      setdiff(trimRedundancies(pkgs)$Package, extractPkgName(c(.RequireDependencies, .basePkgs))),
      ip$Package
    )
    ...
    if (!getOption("Require.usePak") %in% TRUE) {
      ...
      testthat::expect_true(
        sum(grepl("reproducible", out1Attr$Package[out1Attr$installResult %in% "OK"])) == 1 &&
          is.character(getOption("Require.cloneFrom"))
      )
      testthat::expect_true(
        sum(grepl("reproducible", out2Attr$Package[out2Attr$installResult %in% "OK"])) == 0
      )
