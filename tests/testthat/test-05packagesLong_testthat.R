test_that("test 5", {
  ...
  pkgDepTest2 <- Require::pkgDep2(
    c("Require"), # simplify = FALSE,
  ...
  pkgs <- list(
    c(
      ...
      "terra (>=1.7-71)",
      "reproducible (>=2.0.2)",
      "PredictiveEcology/reproducible@development (>=2.0.0)", # Until reproducible 2.0.2 is on CRAN
      "achubaty/amc@development",
      "PredictiveEcology/LandR@development (>=0.0.1)",
      "PredictiveEcology/LandR@development (>=0.0.2)",
      "ianmseddy/LandR.CS (<=0.0.1)"
    ),
    c(
  ...
  for (pkg in pkgs) {
    ...
    testthat::expect_true(all.equal(out, outFromRequire, check.attributes = FALSE))
